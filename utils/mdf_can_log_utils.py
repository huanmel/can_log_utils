...
def convert_log_csv2raw_can_mdf(csv_file):
    ...
    # # log_folder=r'C:\D\proj\gh\can_log_utils\data\csv'
    # # get latest csv file
    # csv_files = glob.glob(os.path.join(log_folder, "*.csv"))
    # csv_file = max(csv_files, key=os.path.getmtime)
    # csv_file=r'C:\Users\ivanm\Documents\MATLAB\EKL\dp187_ravo\dp187_csw\work\test_run\trace.csv'
    # csv_file=r'..\data\csv\CAN_DataFrame_LOG.csv'

    # mdf_file= os.path.join(log_folder,os.path.basename(csv_file).replace('.csv','.mf4'))
    mdf_file= os.path.abspath(csv_file).replace('.csv','.mf4')

    df = pd.read_csv(csv_file,parse_dates=['Time'])
    df['Time'] = pd.to_datetime(df['Time']).dt.tz_localize('UTC')
    flds=["CAN_DataFrame.BusChannel",
    "CAN_DataFrame.ID",
    "CAN_DataFrame.IDE",
    "CAN_DataFrame.DLC",
    ...
    # df['Time'] = df['Time'].str.replace(' sec', '').astype(float)  # Remove 'sec' and convert to float
    # df['Time'] = df['Time'].str.replace(' sec', '').astype(float)  # Remove 'sec' and convert to float
    ...
    # df['Error'] = df['Error'].astype(np.uint8)
    # df['Remote'] = df['Remote'].astype(np.uint8)
    ...
    # t=df['Time'].values
    # bus_channel = np.ones(len(df), dtype='<u1')  # Empty BusChannel (could use 'Name' if populated)
    ...
    # structured_array = np.core.records.fromarrays(arrays)
    ...
    # timestamps=np.arange(len(structured_array))
    t=df['Time']
    starttime=t[0]
    time=t-t[0]
    ...
    # starttime=datetime.datetime(starttime,tzinfo=utc)
    # datetime.tzinfo
...
def extend_can_id(can_id):
    """
    Convert a standard CAN ID to extended format with IDE flag set if needed
    
    Args:
        can_id (int): Original CAN ID (either standard or extended)
    
    Returns:
        int: Properly formatted CAN ID with IDE flag if extended
    """
    if can_id > 0x7FF:
        # For extended frames, set the IDE bit (bit 31)
        return can_id | (1 << 31)
    return can_id
